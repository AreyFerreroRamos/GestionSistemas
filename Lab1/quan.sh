...
if [ $# -eq 0 ]
then
	echo -e "Es requereix al menys un paquet com a paràmetre per dur a terme la funcionalitat." >&2
	exit 1
elif [ $1 = '-h' ]
then
	echo -e "quan.sh : Donat un conjunt de paquets, s'indica quins no están instal·lats i per aquells que estan instal·lats s'indica la data en que es van instal·lar.\n\tParàmetres:\n\t\t- Llista de paquets (Separats per blanc).\n\tRetorn\n\t\t-" >&2
	exit 2
else
	IFS=$' '
	for paquet in $*
